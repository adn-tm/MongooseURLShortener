...
module.exports = function MongooseURLShortener(connection, options) {
    ...
    this.resolve = function (hash, data) {
        ...
        else  Model.findOne({hash: hash}, function (err, res) { resolveDeferred(deferred, err, res); });
        return deferred.promise;
    };
...
function initialize(connection, options) {
    ...
    try {
        ...
        schema.pre('save', function (next) {
            if (!this.hash) {
                this.hash = (options.prefix || "")+perma(this.url, options.length || 6);// shortID.generate();
            }
            next();
        });
